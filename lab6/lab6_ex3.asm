...
READ_LOOP
        GETC
        ADD     R3, R0, #0
        LD      R4, NEWLINE ;input until enter is pressed
        NOT     R4, R4
        ADD     R4, R4, #1
        ADD     R4, R3, R4
        BRz     END_INPUT
...
UP_LOOP
        ...
        ; ADD     R3, R3, #1
        ; ADD     R3, R0, R3
        ; BRn     NO_CHANGE ; skip if below 'a'

        ; LD      R3, ASCII_z
        ; NOT     R3, R3
        ; ADD     R3, R3, #1
        ; ADD     R3, R3, R0
        ; BRp     NO_CHANGE ; skip if above 'z'

        LD      R3, MASK_CLEAR_BIT5 ; convert LOWER CASE to UPPER CASE
        AND     R0, R0, R3
        STR     R0, R2, #0
...
; UPPER CASE decimal < LOWER CASE demcial
